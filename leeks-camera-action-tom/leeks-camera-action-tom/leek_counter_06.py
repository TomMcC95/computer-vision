...
video_name = "vid2_trans_full.mp4" # the name of directory in the input folder should match the dataset name.
output_path = Path.cwd() / output_directory_name / "output.avi"
...
video_start_fraction = 0 # 20000/26500 # Start and end positions of the video (expressed as fraction of total length)
video_end_fraction = 1
...
leek_temp = np.zeros((1000,1000,3), dtype = np.uint8)
leek_tight = np.zeros((10,10,3), dtype = np.uint8)
frames_since_detection = 2
# Making array for storing last n leeks:
leek_store = np.zeros((1000,1000,3), dtype = np.uint8)
leek_result_width = signal_width
leek_result_height = 900
...
while True:
    ...
    if boxes:
        ...
        for idx in range(len(boxes)):
            ...
            leek_temp[500-int(h/2):500-int(h/2)+h ,500-int(w/2):500-int(w/2)+w] = frame_focused[y:y+h,x:x+w]
            M = cv.getRotationMatrix2D((500,500), angle, 1.0) # Parameters: center, angle, scale
